...
function CurrentWeatherComponent() {
  ...
  useEffect(() => {
    ...
    Object.keys(selectedCity).length > 0 && selectedCityId !== cityDataId && fetchCurrentWeatherData();
  }, [selectedCity, cityCurrData]);

  return !isCityCurrLoading && (
    <div className='currentWeather__main margin__top_sm'>
        <div className='currentWeather__cityName'>
            {(selectedCity.nm || '').toUpperCase()}
        </div>
        <div className={`currentWeather__svg margin__top_sm wi wi-icon-${id}`}>
        </div>
        <div className='currentWeather__temperature margin__top_sm'>
          {temp}
        </div>
    </div>
  );
